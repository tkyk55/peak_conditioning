...
from app.app_member.forms import ExBookingForm
        ...
        for staff in staffs:
            ...
            for hour in range(9, 20):
                ...
                for minute in [0, 15, 30, 45]:
                    ...
                    # minute = 0の場合、 hour_flg を1にする
                    if minute == 0:
                        row[minute]['hour_flg'] = 1
...
class StaffMemberListView(LoginRequiredMixin, TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'app/member_list.html', {
            ...
            'year': request.session['pass_prm']['year'],
            'month': request.session['pass_prm']['month'],
            'day': request.session['pass_prm']['day'],
        })
        ...
        if form.is_valid():
            ...
            if booking_exist_data and my_booking_id: # 選択した予約を動かしたい場合
               ...
               if form.is_valid():
                    if form.cleaned_data['booking_id']:
                        booking_old_data = Booking.objects.get(id=int(form.cleaned_data['booking_id']))
                        training_no = form.cleaned_data['training_no']
...
class StaffBookingInputSearchView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        pass_prm = {'training_id': training_id,'training_no': training_no,'year': year, 'month': month, 'day': day, 'hour': hour, 'minute': minute}
...
class StaffClosingModalView(LoginRequiredMixin, TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # トレーニング側作成
        training_detail = training_detail__function(training_data, start_date)
        ...
        start_str = timezone.localtime(datetime_start).strftime('%H:%M')
        hour, minute = start_str.split(':')

        return render(request, 'app/staff_closing_modal_form.html', {
            'form': form,
            'err_cd': err_cd,
            'today': today,
            'training': training_detail,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            # 予約ユーザー存在チェック
            booking_exist_data = Booking.objects.filter(start__gte=start_time_45before, start__lt=start_end_time, training=training_id, del_flg=0)
            ...
            if closing_exist_data:
                ...
                ctx = {
                    ...
                    "err_msg": err_msg,
                    "today": today,
                    "training": Training.objects.filter(del_flg=0).order_by("display_num"),
                    "closing_exist_data": closing_exist_data,  # 既存ブロックがあるならそのまま渡す
                    "click_training_id": training_id,
                    "hour": f"{hour:02d}",
                    "minute": f"{minute:02d}",
                    "overlap_error": True,   # 重複エラー時のみ True
                }
                # ステータスを 400 にすると jQuery の error に入ります（success に入れたいなら 200 のままでも可）
        ...
        ctx = {
            ...
            "err_cd": err_cd,
            "err_msg": err_msg,
            "overlap_error": True,   # ★エラー時のみ True
        }
        # ステータスを 400 にすると jQuery の error に入ります（success に入れたいなら 200 のままでも可）
        return render(request, 'app/staff_closing_modal_form.html', ctx, status=400)
    ...
    def get(self, request, *args, **kwargs):
        ...
        form = ExBookingForm(request.POST or None,
                             initial={
                                 ...
                                 'email_ch': ex_booking_data.email if ex_booking_data else None,
        ...
        if 'update' in request.POST and ex_booking_data: # 更新の場合
            ...
            if form.is_valid():
                ...
                if action_type == 'submit':
                    ...
                    # 日本時間に変換
                    start_jst = start.astimezone(pytz.timezone('Asia/Tokyo'))
                    end_jst = end.astimezone(pytz.timezone('Asia/Tokyo'))

                    context = {
                        'user_name': ex_booking_data.first_name + ' ' + ex_booking_data.last_name,
                        'start_datetime': start_jst.strftime("%Y-%m-%d %H:%M"),
                        'end_datetime': end_jst.strftime("%H:%M"),
                        'site_url': site_url,
                        'shop_tel': shop_tel,
...
def create_empty_timetable():
    ...
    for hour in range(9, 20):
        row = {
            "hour_flag": 0,  # 時間フラグ
            "minutes": {0: 2, 15: 0, 30: 0, 45: 0}  # 分単位
        }
        timetable[hour] = row
    return timetable
        ...
        for duplicates_num in range(training.duplicates_num):
            ...
            for hour in range(9, 20):
                ...
                for minute in [0, 15, 30, 45]:
                    ...
                    # 今日で現在時刻に近い場合、minute_now_flg を1にする
...
# @login_required
# def staff_closing_modal(request, blackgray, closing_id, year, month, day, hour, minute):
#     closing = get_object_or_404(ClosingDay, id=closing_id)
#
#     ic('aaa')
#
#     if year:
...
#     )
#
#     ctx = {"form": form, "closing": closing, "year":year, "month":month, "day":day, "color":blackgray}
#
#     # ▼ Ajaxかpartial指定なら部分テンプレートだけ返す
#     is_partial = (
#             request.headers.get("x-requested-with") == "XMLHttpRequest"
#             or request.GET.get("partial") == "1"
#     )
#
#     ic(f"[DEBUG] is_partial={is_partial} path={request.path}")  # ←ログで確認
#
#     if is_partial:
#         return render(request, "app/staff_closing_modal_form.html", ctx)
#
#     # 通常アクセス時は全ページ
#     return render(request, "staff_closing_day.html", ctx)